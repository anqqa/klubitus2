<!-- #INCLUDE FILE="dbfuncs.asp" -->
<%
Response.Expires = -1000 
if session("admin") = "" then response.redirect "virhe.asp"

lisays = request.form("lisays")
aihe = zeroString(request.form("aihe"))
uutinen = zeroString(request.form("uutinen"))

%>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">

<html>
<head>
	<title>uutiset . lisää</title>
	<link rel="stylesheet" href="vihrea.css" type="text/css">
</head>

<body bgcolor="#002200" background="pics/tausta_vihrea.jpg" bgproperties="fixed">

<%
if lisays = "true" then
	readyDBCon
	Set RS = Server.CreateObject( "ADODB.Recordset" )
	RS.ActiveConnection = Con
	RS.CursorType = adOpenStatic
	RS.LockType = adLockOptimistic
	RS.Open "SELECT * FROM messages", Con, 2, 2
	RS.AddNew
	RS("status") = 11
	RS("topic") = aihe
	RS("author_id") = session("id")
	RS("author") = session("nikki")
	RS("message") = uutinen
	RS("replied") = Now()
	RS("last_reply_id") = session("id")
	RS("last_reply") = session("nikki")
	RS("reply_to") = -1
	RS.Update
	RS.Close
	Con.Close
%>
<table width="100%" cellpadding="5" cellspacing="0" border="0">
<tr>
	<td>
bueno. uutinen lisätty.
<p>
<a href="uutiset.asp" class="navi">takas</a>
	</td>
</tr>
</table>
<% else %>
<form action="uutiset_lisaa.asp" method="post">
<table cellspacing="5" cellpadding="0" border="0" >
<tr>
	<td class="selite">aihe:</td>
	<td><input type="text" maxlength="100" size="50" value="" name="aihe"></td>
</tr>
<tr>
	<td colspan="2">&nbsp;</td>
</tr>
<tr>
	<td class="selite" valign="top">uutinen:</td>
	<td><textarea cols="50" rows="20" name="uutinen"></textarea></td>
</tr>
<tr>
	<td colspan="2">&nbsp;</td>
</tr>
<tr>
	<td align="right" colspan="2">
<input type="hidden" name="lisays" value="true">
<input style="background:transparent" type="image" height="36" alt="ok" width="28" src="pics/icon_ok.gif" border="0" name="ok">&nbsp;<a href="javascript:history.go(-1)"><img height="36" alt="antaa olla" src="pics/icon_pyh.gif" width="28" border="0"></a>
	</td>
</tr>
</table>
</form>
<% end if %>
<div align="right">
<!--WEBBOT bot="HTMLMarkup" startspan ALT="Site Meter" -->
<script type="text/javascript" language="JavaScript">var site="sm3klubitus"</script>
<script type="text/javascript" language="JavaScript1.2" src="http://sm3.sitemeter.com/js/counter.js?site=sm3klubitus">
</script>
<noscript>
<a href="http://sm3.sitemeter.com/stats.asp?site=sm3klubitus" target="_top">
<img src="http://sm3.sitemeter.com/meter.asp?site=sm3klubitus" alt="Site Meter" border=0></a>
</noscript>
<!-- Copyright (c)2000 Site Meter -->
<!--WEBBOT bot="HTMLMarkup" Endspan -->
</div>
</body>
</html>
